...
/* set column headers at first line */
        if(!aoa[0]) aoa[0] = [];
            ...
            aoa[0][1]='Код ';
/* fill data to array */
        for(var i = 0; i < this.MOND_ATYPEArray.length; ++i) {
            if(!aoa[i+1]) aoa[i+1] = [];
            aoa[i+1][0]=this.MOND_ATYPEArray[i].name;
            aoa[i+1][1]=this.MOND_ATYPEArray[i].TheCode;
        }
		/* generate worksheet */
...
,            {wch: 64}
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'MOND_ATYPE');
        

        wb.Props = {
            Title: "Справочник::Тип архива",
            Subject: "Справочник::Тип архива",
            Company: "master.bami",
		...
		/* save to file */
		XLSX.writeFile(wb, 'MOND_ATYPE.xlsx');
	}
    backToList() {
        this.opened = false;
