...
/* set column headers at first line */
        if(!aoa[0]) aoa[0] = [];
            aoa[0][0]='Название';
/* fill data to array */
        for(var i = 0; i < this.moncli_infoArray.length; ++i) {
            if(!aoa[i+1]) aoa[i+1] = [];
            aoa[i+1][0]=this.moncli_infoArray[i].Name;
        }
		/* generate worksheet */
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'moncli_info');
        

        wb.Props = {
            Title: "Организация::Описание",
            Subject: "Организация::Описание",
            Company: "master.bami",
		...
		/* save to file */
		XLSX.writeFile(wb, 'moncli_info.xlsx');
	}
    backToList() {
        this.opened = false;
